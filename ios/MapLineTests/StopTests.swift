...
final class StopTests: XCTestCase {
    ...
    func test_주변응답은정확히일치한POI를가까운후보와구분한다() throws {
        let json = #"""
        {
          "tappedPlace": {
            "kakaoPlaceId": "tapped",
          ...
          "places": [{
            "kakaoPlaceId": "near",
            "name": "가까운 식당",
            "location": { "lat": 37.4978, "lng": 127.0275 },
            "distanceM": 2
          }]
        }
        """#.data(using: .utf8)!

        let decoded = try JSONDecoder().decode(NearbyLookup.Result.self, from: json)

        XCTAssertEqual(decoded.tappedPlace?.name, "스타벅스 강남역7번출구점")
        XCTAssertEqual(decoded.places?.first?.name, "가까운 식당")
    }
