...
손그림 모드를 켜면 오른쪽 `손그림` 버튼이 `손그림 · N획`으로 바뀌며, 바로 아래에 파란
`한 획 되돌리기`와 빨간 `전체 지우기`가 세로로 나타납니다.
...
실제 경로는 지도 도로에 묻히지 않도록 도보 6pt, 대중교통 8pt, 자전거 7pt로 그리고,
경로 요약 글자는 20pt와 7pt 흰 외곽선을 사용합니다. 메모는 1.7배 글자와 7pt 흰 외곽선으로 지도
