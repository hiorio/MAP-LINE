 ...
 *
 * 이동수단마다 외부 API 쿼터가 다르고 자동차는 무료분 이후 비용이 생길 수 있다.
 * 검색과 같은 한도를 주면 한 사람이 짧은 시간에 경로 쿼터를 크게 소모할 수 있다.
 * 구간 모드를 바꾸는 건 사람이 드물게 하는 일이라 이 정도로 충분하다.
 */
 ...
 *
 * 현재 결선 후보가 5곳이므로 6명이면 한 번에 최대 30건이다. 사람이 모임 하나를
 * 짜며 이 계산을 열 번 넘게 누를 일은 없어 10분당 10회로 제한한다.
 */
