...
### 2026-08-06 손그림·경로·메모 가독성 개선 — 최종 보정·CI 통과

- 앱의 오른쪽 `손그림` 버튼을 활성화하면 `손그림 · N획`으로 바뀌고, 바로 아래에 파란
  `한 획 되돌리기`와 빨간 `전체 지우기`를 세로로 표시
- 마지막 획은 반복해서 하나씩 되돌릴 수 있고, 전체 지우기는 5초 `실행 취소`로 복구 가능
- 실제 경로 선은 최종적으로 도보 6pt, 대중교통 8pt, 자전거 7pt로 굵게 조정
- 경로 중간의 이동수단·거리·시간 글자는 17→20pt, 흰 외곽선은 6→7pt로 확대
- 지도 메모는 앱 글자 배율을 1.7배, 흰 외곽선을 7pt로 강화하고, 웹·공유 썸네일도 글자를 bold로
  바꾸고 흰 배경과 테두리 대비를 높임
- 전체 지우기 실행 취소가 다른 장소·메모 변경까지 되감지 않도록 실행 취소 스냅샷을
  변경 종류별로 분리

최종 코드 커밋은 `fdf5d66`입니다. 타입 검사·린트와 관련 렌더링 테스트 17개를 통과했고, macOS CI
실행 `31108320903`에서 Swift 빌드·앱 키·도메인 테스트·전체 UI 테스트·스크린샷 추출까지
모두 통과했습니다. 이 변경은 아직 TestFlight 27에는 포함되지 않았습니다.
...
- 6.5형 스크린샷 5장은 사용자 확인 후 App Store Connect에 업로드할 것
...
손그림 모드를 켜면 오른쪽 `손그림` 버튼이 `손그림 · N획`으로 바뀌고, 그 바로 아래에
`한 획 되돌리기`·`전체 지우기`가 세로로 나타납니다.
