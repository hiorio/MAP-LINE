...
- iOS 지도 기본 POI 탭 작업: 로컬 브랜치 `agent/ios-map-poi-context`, 커밋 `5c3fbe7`.
  스타벅스 같은 카카오 기본 마커 한 번 탭과 빈 지점 롱프레스가 같은 핀 컨텍스트를 열고,
  탭한 POI 또는 좌표상 가장 가까운 검색 결과를 상단에 둔다. **아직 원격 push·macOS CI 전**
...
보관함에 담습니다. `naver.me`/`kko.kr` 단축 URL 자체는 여전히 풀지 않습니다.
