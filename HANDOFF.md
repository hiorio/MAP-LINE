...
- iOS: 번들 ID `com.hiorio.mapline`, 팀 `W297Z9DQ9U`, **TestFlight 1.0 (25) 업로드 완료**
- 원격 `main`은 `9b26c7e`까지 반영돼 메모 직접 드래그, 경로 중간 수단·거리·시간 표시,
  위 사용성 개선 묶음이 CI를 통과했습니다. 이 변경들은 아직 TestFlight 빌드 25에는 없습니다.
...
현재 소스의 메모 롱프레스 드래그 이동과 경로 중간 수단·거리·시간 표시는 빌드 25 이후
변경이라 아직 TestFlight에는 들어가지 않았습니다.
