...
- iOS: 번들 ID `com.hiorio.mapline`, 팀 `W297Z9DQ9U`, **TestFlight 1.0 (23) 업로드 완료**
  (2026-08-06 08:57 KST, Actions run `31057758416` attempt 2)
- iOS 후속 작업: 로컬 브랜치 `agent/ios-map-poi-context`, 커밋 `5c3fbe7`, `524c109`.
  스타벅스 같은 카카오 기본 마커 한 번 탭과 빈 지점 롱프레스가 같은 핀 컨텍스트를 열고,
  탭한 POI 또는 좌표상 가장 가까운 검색 결과를 상단에 둔다. 중간지점 지도 버튼을 없애고,
  보관함에 폴더·마크·색·직접 장소 추가·폴더 이동을 넣었다. **아직 원격 push·macOS CI 전**
