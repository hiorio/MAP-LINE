...
- App Store Connect 앱 이름 `도화지`, 부제 `함께 만드는 모임 동선 지도`, 기본 카테고리
  `내비게이션`, 보조 카테고리 `여행`, 연령 등급 `4+`로 저장
- 개인정보 공개는 공유 지도에 저장되는 `기타 사용자 콘텐츠` 한 항목만 앱 기능 목적으로
  수집하며, 신원 연결과 추적은 하지 않는 것으로 작성
- 공개 개인정보 처리방침 `/privacy`, 지원 `/support`, 한국어 제품 페이지·심사 메모 초안과
  6.5형 스크린샷 5장을 준비. 스크린샷은 사용자 확인 후 App Store Connect에 업로드할 것
- 남은 일: 운영 웹 배포 확인, 스크린샷 사용자 승인, App Review 연락처 입력, 새 빌드 선택,
  수출 규정 질문 확인 후 심사 제출
