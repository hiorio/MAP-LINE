...
코드와 단위·UI 테스트는 작성했으며 macOS CI 전입니다. 새 파일은
`MapDraftStore.swift`, `MapTitleSheet.swift`, `CourseTargetSheet.swift`, `UndoBanner.swift`입니다.
...
- 원격 `main`은 `f67b5dc`까지 반영돼 메모 직접 드래그와 경로 중간 수단·거리·시간 표시가
  CI를 통과했습니다. 이 둘과 위 사용성 개선 묶음은 아직 TestFlight 빌드 25에는 없습니다.
