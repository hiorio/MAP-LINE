...
`npm run typecheck` / `npm run lint` / `npm test`(**301개**) 전부 통과.
...
지도 위 메모는 이제 탭해서 내용을 수정하거나 삭제할 수 있고, `위치 옮기기`를 누른 뒤
새 자리를 꾹 누르면 같은 id를 유지한 채 좌표만 바뀝니다. SDK가 사용자 POI 드래그
이벤트를 주지 않아 지도 팬과 충돌하지 않는 2단계 방식으로 만들었습니다.
...
- 메모 탭 → `위치 옮기기` → 새 자리 꾹 누르기 후 메모가 정확히 옮겨지는가
