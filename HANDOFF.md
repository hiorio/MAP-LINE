...
### 2026-08-06 손그림·경로·메모 가독성 개선 — CI 통과

- 앱의 손그림 모드 아래에 현재 획 수, `한 획 되돌리기`, `전체 지우기`를 함께 표시
- 마지막 획은 반복해서 하나씩 되돌릴 수 있고, 전체 지우기는 5초 `실행 취소`로 복구 가능
- 실제 경로 선은 도보 4→5pt, 대중교통 5→7pt, 자전거 4→6pt로 굵게 조정
- 경로 중간의 이동수단·거리·시간 글자는 17→20pt, 흰 외곽선은 6→7pt로 확대
- 지도 메모는 앱의 흰 외곽선을 4→6pt로 강화하고, 웹·공유 썸네일도 글자를 semibold로
  바꾸고 흰 배경과 테두리 대비를 높임
- 전체 지우기 실행 취소가 다른 장소·메모 변경까지 되감지 않도록 실행 취소 스냅샷을
  변경 종류별로 분리

코드 커밋은 `f36b00a`입니다. 웹 전체 테스트 313개·타입 검사·린트를 통과했고, macOS CI
실행 `31106267539`에서 Swift 빌드·앱 키·도메인 테스트·전체 UI 테스트·스크린샷 추출까지
...
손그림 모드를 켜면 지도 아래에 현재 획 수와 `한 획 되돌리기`·`전체 지우기`가 나타납니다.
