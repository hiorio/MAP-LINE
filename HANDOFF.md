...
### 2026-08-06 보관함·내 지도 관리 묶음 — CI 통과, TestFlight 미반영
...
전체 UI 테스트·스크린샷 추출까지 모두 통과했습니다. **이 묶음은 아직 TestFlight 26에는
들어 있지 않습니다.** 다음 TestFlight 업로드 때 새 빌드 번호로 포함해야 합니다.
...
- iOS: 번들 ID `com.hiorio.mapline`, 팀 `W297Z9DQ9U`, **TestFlight 1.0 (26) 업로드 완료**
- TestFlight 빌드 26에는 `1699ae7`까지의 메모 직접 드래그, 경로 중간 수단·거리·시간 표시와
  첫 번째 사용성 개선 묶음이 들어 있습니다. 보관함·내 지도 관리 묶음(`637f5c5`)은 원격
  `main`과 CI에만 있고 아직 TestFlight에는 없습니다.
...
**TestFlight 1.0 (26)을 App Store Connect에 업로드했습니다.** Apple 처리 완료 후
