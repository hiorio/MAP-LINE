...
앱의 동선 화면에서도 각 단계 아래 `이 단계에 후보 추가`를 눌러 장소를 검색해 같은
번호에 여러 후보를 담을 수 있습니다. 단계 수와 id는 유지되고 후보 배열만 늘어나며,
대표가 이미 정해져 있으면 그 선택도 유지합니다.
...
32pt → 28pt(이름 22pt → 20pt), 롱프레스는 UIKit 인식기로 0.45초에 열립니다.
...
- 28pt 단계 핀과 20pt 이름표가 실제 화면에서 너무 작거나 크지 않은가
- 메모 탭 → `위치 옮기기` → 새 자리 꾹 누르기 후 메모가 정확히 옮겨지는가
- 동선에서 `이 단계에 후보 추가` → 검색 → 같은 번호의 핀이 둘 이상 생기는가
